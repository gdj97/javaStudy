 ...
 *   int read(byte[] buf) : buf의 크기만큼 읽어서 데이터는 buf에 저장. 실제읽은 바이트수 리턴
 *   int read(byte[] buf,int offset,int len) : 
 *              len크기만큼 읽어서 데이터는 buf의 offset 인덱스부터 저장. 실제 읽은 바이트수 리턴
...
public class MainEx01_fileinputstream {
	public static void main(String[] args) throws IOException {
		...
		while((data = fis.read(buf)) != -1) {  //data : 읽은 바이트 수. buf:파일의 내용
			//new String(buf,0,data) : buf의 내용을 0번인덱스부터 data크기까지 문자열로 생성
