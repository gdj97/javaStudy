...
/*
 * Thread : 3초에 한번씩 List에 단어를 추가함. Daemon 스레드로 생성
 * main   : List에 추가된 단어를 입력받아서 제거함. 
 * => main이 List의 모든 단어를 제거하면 main 종료. 
 */
...
public class MainEx02_wordgame {
	public static void main(String[] args) {
		...
		t1.start();
		while (true) {
			System.out.println(words); //Thread가 추가한 데이터 출력
			System.out.print(">>");
			String input = scan.next().trim();
			words.remove(input); //input에 해당되는 데이터를 words 에서 제거
