 ...
 * 값 get(Key) : key값을 매개변수로 입력하면, key에 해당하는 value 값을 리턴.
 *              key값이 없는 경우 null 리턴
 * 값 remove(Key) : key값을 매개변수로 입력하면, Map.Entry객체를 제거하고, value값을 리턴
 *              key값이 없는 경우 null 리턴
 *              
 * Map.Entry<K,V> : (key,value)쌍인 객체의 자료형                          
 *      K getKey()   : (key,value)쌍인 객체 중 key값 리턴
 *      V getValue() : (key,value)쌍인 객체 중 value값 리턴
 *      
 * Value 값으로 Key값 조회 방법은 없다.     
 */
