...
//		lday.set(year, mon - 1,lastdate); //=> 현재월의 마지막일자로 설정
		...
		System.out.printf("%4d-%02d-%02d %c요일\n",
			fday.get(Calendar.YEAR),fday.get(Calendar.MONTH)+1,fday.get(Calendar.DATE),
			//fday.get(Calendar.DAY_OF_WEEK) : 1(일) ~ 7(토)
			week.charAt(fday.get(Calendar.DAY_OF_WEEK)-1));
		System.out.printf("%4d-%02d-%02d %c요일\n",
				lday.get(Calendar.YEAR),lday.get(Calendar.MONTH)+1,lday.get(Calendar.DATE),
				week.charAt(lday.get(Calendar.DAY_OF_WEEK)-1));
